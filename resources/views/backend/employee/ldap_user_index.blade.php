...
<div>
    <div class="d-flex justify-content-between pb-3">
        <div class="grow">
            <h4 class="text-20">@lang('LDAP Users List')</h4>
        </div>
...
@push('after-scripts')
    <script>
        $(document).ready(function() {
            ...
            var route = '{{ route('admin.employee.get_ldap_data', ['status' => $status]) }}';

            @if (request('show_deleted') == 1)
                route = '{{ route('admin.employee.get_ldap_data', ['show_deleted' => 1]) }}';
            @endif
