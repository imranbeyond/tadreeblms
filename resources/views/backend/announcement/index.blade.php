...
@section('title', 'Announcement'.' | '.app_name())
@push('after-styles')
...
@push('after-scripts')
    <script>
        ...
        $(document).ready(function () {
            var route = '{{route('admin.announcement.get_data')}}';

            @if(request('show_deleted') == 1)
                route = '{{route('admin.announcement.get_data',['show_deleted' => 1])}}';
            @endif
    ...
    $('#myTable_length select').addClass('form-select form-select-sm custom-entries');
       ...
       search:"",
        ...
        $(document).on('click', '.switch-input', function (e) {
            ...
            $.ajax({
                type: "POST",
                url: "{{ route('admin.announcement.status') }}",
                data: {
                    _token:'{{ csrf_token() }}',
                    id: id,
                },
